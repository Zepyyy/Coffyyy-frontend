...
const methodTones: Record<string, string> = {
	Espresso:
		"bg-amber-100/80 text-amber-950 dark:bg-amber-200/15 dark:text-amber-100",
	"Pour over": "bg-sky-100 text-sky-950 dark:bg-sky-200/15 dark:text-sky-100",
	"Moka pot":
		"bg-orange-100 text-orange-950 dark:bg-orange-200/15 dark:text-orange-100",
	Immersion:
		"bg-violet-100 text-violet-950 dark:bg-violet-200/15 dark:text-violet-100",
};
				...
				<nav className="flex items-center gap-1 border-b border-foreground/10">
					{(["beans", "brewers"] as Section[]).map((item) => (
						...
						>
							{item}
							{section === item && (
								<span className="absolute inset-x-0 bottom-px border-b border-foreground" />
							)}
						</button>
					))}
				</nav>
		...
		<div className="space-y-3">
			...
			{showFilters && (
				<div className="flex flex-wrap items-center gap-2 border-y border-foreground/10 py-3">
					...
					{(section === "beans"
						? ["Ethiopia", "Costa Rica", "Colombia"]
						: ["Espresso", "Pour over", "Moka pot"]
					).map((filter) => (
						<button
							key={filter}
							type="button"
							className="border border-foreground/15 px-2.5 py-1 font-Recursive text-xs hover:border-primary"
						>
							{filter}
						</button>
					))}
					<span className="ml-auto font-Mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">
						Brand available below
					</span>
				</div>
			)}
		</div>
	...
	return (
		<div
			className={cn(
				...
				mode === "rail" && "flex-col items-start gap-1",
				mode === "lead" && "border-l-2 border-current pl-3",
			)}
		>
			<div className="flex items-baseline gap-2">
				...
				<span className="font-Mono text-[9px] uppercase tracking-widest opacity-65">
					{brewer.methodCue}
				</span>
			</div>
...
function BrewPreview({
	...
	return (
		<div className="fixed inset-x-3 bottom-4 z-50 mx-auto max-w-xl border border-foreground bg-background p-4 shadow-2xl sm:inset-x-auto sm:right-6 sm:left-auto sm:w-md">
			<div className="flex items-start justify-between gap-4">
				<div>
					<p className="font-Mono text-[9px] uppercase tracking-[0.18em] text-primary-800/60 dark:text-primary-200/60">
						Prototype brew setup
					</p>
					<h2 className="mt-1 font-News text-2xl">Start a brew</h2>
				</div>
				<button type="button" onClick={close} aria-label="Close brew setup">
			...
			<div className="mt-4 grid grid-cols-3 gap-2 border-y border-foreground/10 py-3">
				<div>
					...
					<p className="mt-1 truncate font-Recursive text-sm">
						{section === "beans" ? item.name : "Choose a bean"}
					</p>
				</div>
				<div>
					<p className="font-Mono text-[9px] uppercase text-muted-foreground">
					...
					<p className="mt-1 font-Recursive text-sm">Choose method</p>
				</div>
				<div>
					<p className="font-Mono text-[9px] uppercase text-muted-foreground">
						Brewer
					...
					<p className="mt-1 truncate font-Recursive text-sm">
						{section === "brewers" ? item.name : "Optional"}
					</p>
				</div>
			</div>
			<button
				type="button"
				onClick={close}
				className="mt-4 flex w-full items-center justify-center gap-2 bg-foreground px-4 py-2.5 font-Mono text-[10px] uppercase tracking-[0.14em] text-background"
			>
				Continue to brew log <ArrowRight className="size-3.5" />
			</button>
		</div>
	);
}
...
function BrewerActions({
	...
	brewer: Brewer;
	startBrew: (item: Brewer) => void;
}) {
	return (
		<div className="flex items-center justify-between gap-3 border-t border-foreground/10 pt-4">
			<button
				type="button"
				className="font-Mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground underline decoration-dotted underline-offset-4"
			>
				View details
			</button>
			<button
...
function BeanActions({
	bean,
	startBrew,
}: {
	bean: Bean;
	startBrew: (item: Bean) => void;
}) {
	return (
		<div className="flex items-center justify-between gap-3 border-t border-foreground/10 pt-4">
			<button
	...
	return (
		<article className="flex min-h-72 flex-col overflow-hidden border border-foreground/10 bg-background/75">
			<div className={cn("min-h-32 p-5", methodTones[brewer.method])}>
				...
				<p className="mt-1 font-Recursive text-sm opacity-75">
			...
			<div className="flex flex-1 flex-col p-5">
				...
				<div className="mt-auto pt-5">
					<BrewerActions brewer={brewer} startBrew={startBrew} />
				</div>
			</div>
		</article>
	);
...
function BeanCardA({
	...
	return (
		<article className="flex min-h-72 flex-col overflow-hidden border border-foreground/10 bg-background/75">
			<div
				className={cn(
					"relative min-h-32 overflow-hidden bg-linear-to-br p-5",
					beanColors[index % beanColors.length],
				)}
			>
				<div className="absolute -right-3 -top-6 font-Lora text-8xl italic text-foreground/10">
					{index + 1}
				</div>
				<p className="relative font-Mono text-[9px] uppercase tracking-[0.16em] text-foreground/55">
					{bean.origin} · {bean.roast} roast
				</p>
				<h2 className="relative mt-6 max-w-56 font-Lora text-2xl leading-none text-foreground">
					{bean.name}
				</h2>
			</div>
			<div className="flex flex-1 flex-col p-5">
...
function AddCard({ section }: { section: Section }) {
	return (
		<button
			type="button"
			className="flex min-h-72 flex-col items-center justify-center gap-3 border border-dashed border-foreground/20 text-muted-foreground transition-colors hover:border-foreground hover:text-foreground"
		>
			<Plus className="size-5" />
			<span className="font-Mono text-[10px] uppercase tracking-[0.14em]">
				Add {section === "beans" ? "bean" : "brewer"}
			</span>
		</button>
	);
}
...
function useCollection(section: Section) {
	...
	const items = source.filter(
		(item) =>
			...
			`${item.name} ${section === "beans" ? `${(item as Bean).roaster} ${(item as Bean).origin}` : `${(item as Brewer).brand} ${(item as Brewer).model} ${(item as Brewer).method}`}`
				.toLowerCase()
...
function PrototypeFrame({
	...
	return (
		<LibraryShell section={section} setSection={setSection}>
			{children}
		</LibraryShell>
	);
}
...
function VariantA({
	...
	section: Section;
	setSection: (section: Section) => void;
	startBrew: (item: Bean | Brewer) => void;
}) {
		...
		<PrototypeFrame section={section} setSection={setSection}>
			<div className="mb-6 flex items-center justify-between gap-4">
				<p className="font-Recursive text-sm text-muted-foreground">
					Everything useful, at a glance.
				</p>
				<button
					type="button"
					className="hidden items-center gap-2 border border-foreground/15 px-3 py-2 font-Mono text-[10px] uppercase tracking-[0.12em] sm:flex"
				>
					<Plus className="size-3.5" /> Add{" "}
					{section === "beans" ? "bean" : "brewer"}
				</button>
			</div>
			<CollectionTools section={section} {...collection} />
			<div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{collection.ordered.map((item, index) =>
					section === "beans" ? (
						<BeanCardA
							key={item.id}
							bean={item as Bean}
...
function WideCard({
	...
	if (section === "beans") {
		...
		return (
			<article className="grid gap-5 border-b border-foreground/10 py-5 md:grid-cols-[10rem_minmax(0,1fr)_auto] md:items-center">
				<div
					className={cn(
						"flex min-h-28 flex-col justify-between bg-linear-to-br p-4",
						beanColors[BEANS.indexOf(bean) % beanColors.length],
					)}
				>
					<span className="font-Mono text-[9px] uppercase tracking-[0.15em] text-foreground/60">
						{bean.origin}
					</span>
					<span className="font-Lora text-2xl leading-none text-foreground">
						{bean.name}
					</span>
				</div>
				<div>
					<div className="flex flex-wrap items-baseline gap-2">
						<h2 className="font-News text-2xl">{bean.name}</h2>
						<span className="font-Recursive text-sm text-muted-foreground">
							{bean.roaster}
						</span>
					</div>
					<div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
						<Detail
							label="Roast / process"
							value={`${bean.roast} · ${bean.process}`}
						/>
						<Detail label="Notes" value={bean.note} />
						<Detail
							label="Last brewed"
							value={`${bean.lastUsed} · ${bean.lastMethod}`}
						/>
				...
				<div className="flex gap-2 md:flex-col">
					<button
						type="button"
						className="border border-foreground/15 px-3 py-2 font-Mono text-[10px] uppercase tracking-widest"
					>
						Details
					</button>
					<button
						type="button"
						onClick={() => startBrew(bean)}
						className="bg-foreground px-3 py-2 font-Mono text-[10px] uppercase tracking-widest text-background"
					>
						Brew
					</button>
				</div>
	...
	return (
		<article className="grid gap-5 border-b border-foreground/10 py-5 md:grid-cols-[10rem_minmax(0,1fr)_auto] md:items-center">
			<div
				className={cn(
					"flex min-h-28 flex-col justify-between p-4",
					methodTones[brewer.method],
				)}
			>
				<MethodLabel brewer={brewer} mode="rail" />
				<span className="font-News text-2xl leading-none">{brewer.name}</span>
			</div>
			<div>
				<div className="flex flex-wrap items-baseline gap-2">
					<h2 className="font-News text-2xl">{brewer.name}</h2>
					<span className="font-Recursive text-sm text-muted-foreground">
						{brewer.brand} · {brewer.model}
					</span>
				</div>
...
function VariantB({
	...
	return (
		<PrototypeFrame section={section} setSection={setSection}>
			<div className="mb-6 flex items-center justify-between gap-4">
				<p className="font-Recursive text-sm text-muted-foreground">
					A little more breathing room for the details.
				</p>
				<button
					type="button"
					className="flex items-center gap-2 border border-foreground/15 px-3 py-2 font-Mono text-[10px] uppercase tracking-[0.12em]"
				>
					<Plus className="size-3.5" /> Add
				</button>
			</div>
			<CollectionTools section={section} {...collection} />
			<div className="mt-6">
				{collection.ordered.map((item) => (
					<WideCard
						key={item.id}
						item={item as Bean | Brewer}
						section={section}
						startBrew={startBrew}
					/>
				))}
...
function LeadCard({
	...
	if (section === "beans") {
		...
		return (
			<article
				className={cn(
					"flex flex-col overflow-hidden border border-foreground/10 bg-background/75",
					lead ? "min-h-96 sm:col-span-2" : "min-h-72",
				)}
			>
				<div
					className={cn(
						"relative flex flex-1 flex-col justify-end overflow-hidden bg-linear-to-br p-6",
						beanColors[BEANS.indexOf(bean) % beanColors.length],
					)}
				>
					<div className="absolute -right-6 -top-10 font-Lora text-[11rem] italic leading-none text-foreground/10">
						{BEANS.indexOf(bean) + 1}
					</div>
					<p className="relative font-Mono text-[10px] uppercase tracking-[0.16em] text-foreground/60">
						{bean.origin} · {bean.roast} roast
					</p>
					<h2
						className={cn(
							"relative mt-5 font-Lora leading-[0.9] text-foreground",
							lead ? "text-5xl" : "text-3xl",
						)}
					>
						{bean.name}
					</h2>
	...
	return (
		...
		>
			...
			>
				<MethodLabel brewer={brewer} mode="lead" />
				<div>
					<h2
						className={cn(
							"mt-10 font-News leading-[0.9]",
							lead ? "text-5xl" : "text-3xl",
						)}
					>
						{brewer.name}
					</h2>
					<p className="mt-2 font-Recursive text-sm opacity-75">
						{brewer.brand} · {brewer.model}
					</p>
				</div>
			</div>
...
function VariantC({
	...
	return (
		<PrototypeFrame section={section} setSection={setSection}>
			<div className="mb-6 flex items-center justify-between gap-4">
				<p className="font-Recursive text-sm text-muted-foreground">
					One item gets more visual weight; everything stays actionable.
				</p>
				<button
					type="button"
					className="flex items-center gap-2 border border-foreground/15 px-3 py-2 font-Mono text-[10px] uppercase tracking-[0.12em]"
...
function PrototypeSwitcher({
	...
	return (
		<div className="fixed inset-x-0 bottom-4 z-60 mx-auto flex w-fit items-center gap-4 border border-foreground bg-foreground px-3 py-2 text-background shadow-2xl">
			<button
				type="button"
				onClick={() => cycle(-1)}
			...
			<div className="min-w-52 text-center">
				...
				<p className="font-Recursive text-xs">
					{active.name}{" "}
					<span className="text-background/60">· {active.note}</span>
				</p>
			</div>
...
export default function LibraryPrototype() {
	...
	const [section, setSection] = useState<Section>(
		searchParams.get("section") === "brewers" ? "brewers" : "beans",
	);
	...
	return (
		<>
			<div className="mb-5 border border-dashed border-primary/30 bg-primary-700/5 px-4 py-2.5 text-center">
				<p className="font-Mono text-[9px] uppercase tracking-[0.18em] text-primary-800/70 dark:text-primary-200/70">
					Throwaway prototype · round 2 · read-only sample data
				</p>
			</div>
			{content}
			{brewItem && (
