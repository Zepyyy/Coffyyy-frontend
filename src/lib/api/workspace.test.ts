...
import { replaceLocalSnapshot, snapshotHash, validateSnapshot, type WorkspaceSnapshot } from "./workspace";

const snapshot: WorkspaceSnapshot = {
	schemaVersion: 1,
	beans: [{ localId: "bean-1", name: "Ethiopia", rating: 0, status: "New", dominantNote: "Fruity", roastLevel: 3, origin: [], process: [], variety: [], brand: "", botanic: "Arabica", designation: "Pure Origin", flavors: [], finished: false }],
	machines: [{ localId: "machine-1", name: "Linea Mini", brand: "La Marzocco", type: "Espresso", purchaseDate: "", model: "", grindRange: "", capacity: "" }],
	brews: [{ localId: "brew-1", beanLocalId: "bean-1", machineLocalId: "machine-1", beanWeight: 18, espressoWeight: 36, extractionTime: "30s", flow: "Perfect", overallRating: 4, tasteScore: 1, strengthScore: 0, grindSize: 12, date: "2026-07-31T00:00:00.000Z" }],
};

describe("workspace snapshot seam", () => {
	beforeEach(async () => {
		await db.open();
		await Promise.all([db.Beans.clear(), db.Machines.clear(), db.Brews.clear()]);
	});
	...
	it("hashes the same content independent of record order", () => {
		const reordered = { ...snapshot, beans: [...snapshot.beans].reverse(), machines: [...snapshot.machines].reverse(), brews: [...snapshot.brews].reverse() };
